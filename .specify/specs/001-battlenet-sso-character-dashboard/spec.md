...
- Q: FR-016 forbids caching character data. Does that reach the guild roster, which the guild overview shows on the site's home page? → A: No, and the distinction is worth stating. FR-016 exists so nobody is shown stale character state — gear, item level, last login — which changes every time somebody plays. A guild's membership list changes in days, and the roster is what every member lands on, so fetching it afresh on every page load makes the front page wait on Blizzard for data that has not moved. It is cached with a short lifetime instead.
...
- **FR-018**: The system MAY cache the guild roster for a bounded lifetime, since
  guild membership changes over days rather than between page views and the
  roster is displayed on the site's home page. The lifetime MUST be bounded and
  configurable. Where a refresh fails and a previously fetched roster is held,
  the system SHOULD serve that rather than an error: a roster an hour old is a
  better answer than an empty guild. This does not relax FR-016, which governs
  character data, nor FR-013a, whose check runs per request on a separate path.
