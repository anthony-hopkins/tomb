...
| `TOMB_DISCORD_INVITE` | the guild's Discord invite link, shown on the public front door. An `https://` link; the app refuses to start on anything else. Optional; unset means the page says to ask an officer | all |
...
> `TOMB_GUILD_ROSTER_TTL`, `TOMB_GUILD_OFFICER_RANK`, `TOMB_TIMEZONE` and
> `TOMB_DISCORD_INVITE` travel
